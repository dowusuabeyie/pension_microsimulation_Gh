...
AGE_LABEL_FONTSIZE = 7  # 👈 control font size here
...
def make_contributor_pyramid(year):
    ...
    if age_order is None:
        ...
        age_order_local = [a for a in age_order if not a.startswith("60")]
    ...
    color_m, color_f = "#1f77b4", "#ff7f0e"  # same as before

    axes[0].barh(y, micro_pct.get("M", 0), color=color_m, alpha=0.8, label="Male")
    axes[0].barh(y, micro_pct.get("F", 0), color=color_f, alpha=0.8, label="Female")
    ...
    plt.close(fig)
    print(f"[{year}] contributors saved → {outp}")

for y in years:
    make_contributor_pyramid(y)
print(" All contributor pyramids saved to ./figures.")